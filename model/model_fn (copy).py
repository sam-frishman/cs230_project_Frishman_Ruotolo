...
def build_model(is_training, inputs, params):
    ...
    #convd(input, filter, strides, padding, use_cudnn_on_gpu=True)
    # layer 1, i = 0?, channels = 1, filter dim = 4
    with tf.variable_scope('block_{}'.format(1)):
	out = tf.layers.conv2d(out, 4, 2, padding='same')
	out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
	out = tf.nn.relu(out)
	out = tf.layers.max_pooling2d(out, 2, 2)

    # layer 1, i = 1, channels = 1, filter dim = 4
    with tf.variable_scope('block_{}'.format(2)):
	out = tf.layers.conv2d(out, 5, 3, padding='same')
	out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
	out = tf.nn.relu(out)
	out = tf.layers.max_pooling2d(out, 2, 2)

    # layer 1, i = 1, channels = 1, filter dim = 4
    with tf.variable_scope('block_{}'.format(3)):
	out = tf.layers.conv2d(out, 2, 3, padding='same')
	out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
	out = tf.nn.relu(out)
	out = tf.layers.max_pooling2d(out, 2, 2)
